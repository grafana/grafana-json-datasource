...
describe('datasource', () => {
  ...
  it('should throw error when method is not POST or GET', async () => {
    ...
    const responsePUT = ds.doRequest({ urlPath: 'http://localhost:3000/', method: 'PUT' } as any);

    expect(responsePUT).rejects.toThrowError('Invalid method PUT');

    const responsePATCH = ds.doRequest({ urlPath: 'http://localhost:3000/', method: 'PATCH' } as any);

    expect(responsePATCH).rejects.toThrowError('Invalid method PATCH');

    const responseDELETE = ds.doRequest({ urlPath: 'http://localhost:3000/', method: 'DELETE' } as any);
